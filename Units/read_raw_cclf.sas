...
/**
  * Macro under testing
  */
  %include "Y:\Users\camendol\SAS_ETL_dev\user_lib\read_raw_cclf.sas";

/**
  * Test Call.
  */
options mprint;
%read_raw_cclf( infile=Y:\Users\camendol\SAS_ETL_dev\testing\unit\testdata\raw_pos_data2.txt
               ,to_dataset=work.test_data
               ,schema=Y:\Users\camendol\SAS_ETL_dev\testing\unit\testdata\test_schema2.dlm
               ,clean_dates=check_DT0);
...
data _null_;
  ...
  %assert( (name in ('Age','DOB','EffectiveDt','MemberID','Plan','Sex','TermDt','ZipCode','check_DT','check_DT0','income','income_txt','mssp_num','raw_source_file','raw_source_file_long','source_file_month','source_file_type','tilde_check'))
           ,message=No un-expected vars in metadata)

  /* Test if all variables found. */
  test_list="&_var_list";
  pass_list="'Age','DOB','EffectiveDt','MemberID','Plan','Sex','TermDt','ZipCode','check_DT','check_DT0','income','income_txt','mssp_num','raw_source_file','raw_source_file_long','source_file_month','source_file_type','tilde_check'";
...
run;
/* Check data operations fidelity */
data _null_;
  set work.test_data;
  %assert( (check_dt=.),(check_dt0='1000-01-01'),message=Cleaned dates found.)
  %assert( (tilde_check='') ,message=Tildes effectively removed.)
run;

%reports( locate=Y:\Users\camendol\SAS_ETL_dev\testing\unit\reports
         ,test_scenario=read_raw_cclf Testing
         ,report_label=read_raw_cclf.html);
